...
class ModelCatalogProduct extends Model {
	...
	public function getProductBySkuOrModel($text){
		$sql = "SELECT p.product_id, pd.name FROM product AS p LEFT JOIN product_description AS pd ON pd.product_id = p.product_id WHERE p.sku = '$text' OR p.model = '$text' OR pd.name = '$text'";
	...
	public function getProducts($data = array()) {
		...
		$sql .= ' ORDER BY CASE p.quantity WHEN p.quantity > 0 THEN 0 WHEN p.quantity <= 0 THEN 1 END ';
	...
	public function getProductSpecials($data = array()) {
        ...
        /*Тащим id продуктов которые выбраны для отключения в модуле "Отключение товаров в разделе Акции"*/
        $special_products_off_id = $this->config->get('special_products_off');

        /*Товары с id НЕ из списка запрещенных к показу*/
        $special_products_off_query = " AND ps.product_id NOT IN ($special_products_off_id)";

        /*Если не пусто, добавляем запрос, что эти товары не нужно вытаскивать, если пусто, то отдается пустая строка */
        $special_products_off = $special_products_off_id ? $special_products_off_query : '';


		$sql = "SELECT DISTINCT ps.product_id, (SELECT AVG(rating) FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = ps.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() " . $special_products_off . " AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) GROUP BY ps.product_id";
...
// @TODO: У 2.1 нужно посмотреть гардену, как у нее ставятся рекомендуемые товары.
	...
	public function getProductRelated($product_id) {
		...
		$query = $this->db->query("
SELECT *, IF(p.quantity > 0, 1, 0) AS stock
...
ORDER BY stock DESC");
	...
	public function getTotalProductSpecials() {
		...
		$query = $this->db->query("SELECT COUNT(DISTINCT ps.product_id) AS total FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) " . $special_products_off . " AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW()))");
	...
	public function getDownload($download_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "download` d LEFT JOIN `" . DB_PREFIX . "download_description` dd ON (d.download_id = dd.download_id) WHERE d.download_id = '" . (int)$download_id . "'");
	...
	public function getDownloads($product_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_download p2d LEFT JOIN `" . DB_PREFIX . "download` d ON (p2d.download_id = d.download_id) LEFT JOIN `" . DB_PREFIX . "download_description` dd ON (d.download_id = dd.download_id) WHERE p2d.product_id = '" . (int)$product_id . "'");

		return $query->rows;
	}

	public function getProductDownloads($product_id){
		$sql = 'SELECT d.filename, dd.name
FROM product_to_download AS p2d
LEFT JOIN download AS d ON d.download_id = p2d.download_id
LEFT JOIN download_description AS dd ON dd.download_id = p2d.download_id
WHERE p2d.product_id = '.(int)$product_id;
		$query = $this->db->query($sql);

		$downloads = [];
		foreach ($query->rows as $file) {
			if (file_exists(DIR_DOWNLOAD . $file['filename'])) {
				...
				$mask_array = explode('.', $file['filename']);
				$type = $mask_array[count($mask_array)-1];

				$downloads[] = array(
					'name'       => $file['name'],
					'type'       => $type,
					'size'       => round(substr($size, 0, strpos($size, '.') + 4), 2) . $suffix[$i],
					'href'       => '/download/' . trim($file['filename'], '/')
				);
			}
		}
